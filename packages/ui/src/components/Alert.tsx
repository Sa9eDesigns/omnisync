// Alert Component using Cross-Platform Styled System
...
export const AlertTitle = createStyledComponent(
  isWeb ? 'div' : 'Text',
  {
    name: 'AlertTitle',
    defaultProps: {
...
export const AlertDescription = createStyledComponent(
  isWeb ? 'div' : 'Text',
  {
...
const getDefaultIcon = (variant: string) => {
  ...
  if (isWeb) {
    ...
    // React Native - using text emojis for simplicity
    const { Text } = require('react-native');
    switch (variant) {
      case 'info':
        return <Text style={{ fontSize: 16 }}>ℹ️</Text>;
      case 'success':
...
export const Alert = forwardRef<any, AlertProps>(
  (
    ...
    const iconToShow = icon !== undefined ? icon : getDefaultIcon(variant);
